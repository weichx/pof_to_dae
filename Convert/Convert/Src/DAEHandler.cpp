...
void DAEHandler::process_subobj(const pugi::xml_node& element, int parent, matrix rotation_matrix) {
	...
	if (!geom) {
		std::cout << "instance_geometry not found for subobject " << get_name(element) << std::endl;
		return;
	}
	progress->incrementWithMessage("Processing ___ " + get_name(element));
...
DAEVectorInput::DAEVectorInput(pugi::xml_node element) {
	...
	if (found != 3) {
		std::cout << "Incorrect number of coordinate axes found!" << std::endl;
		return;
	}
...
DAEUvInput::DAEUvInput(pugi::xml_node element) {
	...
	if (found != 2) {
		cout << "Incorrect number of coordinate axes found!" << std::endl;
		return;
	}
	next = element.find_child_by_attribute("id", &next.attribute("source").value()[1]);
	if (!next) {
...
void DAESaver::add_header() {
	...
	up.text().set("Y_UP"); //todo -- see if setting Y_UP will fix needing unity to do rotation by vertex
}
...
void DAESaver::add_geom() {
	...
	string temp("detail0");
	pugi::xml_node detail = scene.append_child("node");
	detail.append_attribute("id") = "detail";
	detail.append_attribute("name") = "detail";
	//mabye need a transform node?
	std::vector<pcs_sobj> model_subobjects = model->get_subobjects();

	for (int i = 0; i < model->GetLODCount(); i++) {
		temp[strlen("detail")] = i + '0';
		pugi::xml_node detail_node;
		write_geometry(temp, model->LOD(i), model_subobjects[model->LOD(i)], detail, detail_node);
	}

	pugi::xml_node debris = scene.append_child("node");
	debris.append_attribute("id") = "debris";
	debris.append_attribute("name") = "debris";
	temp = "debris01";
	for (int i = 0; i < model->GetDebrisCount(); i++) {
		temp[strlen("debris0")] = ((i + 1) % 10) + '0';
		temp[strlen("debris")] = ((i + 1) / 10) + '0';
		pugi::xml_node debris_node;
...
void DAESaver::write_firepoints(pcs_turret &turret_instance, pugi::xml_node &node) {
	for (int i = 0; i < turret_instance.fire_points.size(); i++) {
		string name = "firepoint" + to_string(i);
		pugi::xml_node firepoint_node = node.append_child("node");
		firepoint_node.append_attribute("id") = name.c_str();
		firepoint_node.append_attribute("name") = name.c_str();
		write_transform(firepoint_node, turret_instance.fire_points[i], turret_instance.turret_normal, { 0, 1, 0 });
	}
}
...
void DAESaver::write_geometry(string &name, int idx, pcs_sobj &obj, pugi::xml_node &parent_node, pugi::xml_node& out_node) {
	...
	translate.text().set(write_vector3d(obj.offset).c_str());
	...
	for (unsigned int i = 0; i < obj.polygons.size(); i++) {
		if (obj.polygons[i].texture_id < num_textures && obj.polygons[i].texture_id >= 0) {
			polies[obj.polygons[i].texture_id].push_back(&(obj.polygons[i]));
			counters[obj.polygons[i].texture_id]++;
		}
		else {
			polies[num_textures].push_back(&(obj.polygons[i]));
	...
	temp = out_node.insert_child_after("instance_geometry", translate);
...
void DAESaver::get_subobj(int idx, string *name) {
	...
	if (sobj.parent_sobj == -1) {
		...
		//otherwise if it has a a parent subobject, make it a child of that
	...
	pugi::xml_node translate = subobj.prepend_child("translate");
	translate.text().set(write_vector3d(sobj.offset).c_str());
	//if a name is given, use that name as the node name, otherwise use the name of the subobject itself
	if (name) {
		//progress->incrementWithMessage("Adding SubObject - " + *name);
		...
		subobj.append_attribute("id") = sobj.name.c_str();
		subobj.append_attribute("name") = sobj.name.c_str();
		//progress->incrementWithMessage("Adding SubObject - " + sobj.name);
	}
